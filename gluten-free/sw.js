const CACHE_NAME = "gluten-free-365-v10";
const APP_SHELL = [
  "/",
  "/index.html",
  "/data.js",
...
self.addEventListener("fetch", (event) => {
  ...
  const isAppShell = url.origin === self.location.origin && (url.pathname === "/" || url.pathname.startsWith("/gluten-free/"));
  ...
  if (isAppShell || event.request.mode === "navigate") {
    event.respondWith(
      fetch(event.request)
        ...
        .catch(() => caches.match(event.request).then((cached) => cached || caches.match("/") || caches.match("/gluten-free/index.html")))
    );
    return;
  }
